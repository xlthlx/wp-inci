...
class CMB2_JS {
	...
	protected static function colorpicker_frontend() {
		...
		wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
			'clear'         => esc_html__( 'Clear', 'wp-inci' ),
			'defaultString' => esc_html__( 'Default', 'wp-inci' ),
			'pick'          => esc_html__( 'Select Color', 'wp-inci' ),
			'current'       => esc_html__( 'Current Color', 'wp-inci' ),
		) );
	}
		...
		$l10n = array(
			...
			'defaults'          => array(
				...
				'date_picker'  => array(
					'changeMonth'     => true,
					'changeYear'      => true,
					'dateFormat'      => _x( 'mm/dd/yy', 'Valid formatDate string for jquery-ui datepicker', 'wp-inci' ),
					'dayNames'        => explode( ',', esc_html__( 'Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday', 'wp-inci' ) ),
					'dayNamesMin'     => explode( ',', esc_html__( 'Su, Mo, Tu, We, Th, Fr, Sa', 'wp-inci' ) ),
					'dayNamesShort'   => explode( ',', esc_html__( 'Sun, Mon, Tue, Wed, Thu, Fri, Sat', 'wp-inci' ) ),
					'monthNames'      => explode( ',', esc_html__( 'January, February, March, April, May, June, July, August, September, October, November, December', 'wp-inci' ) ),
					'monthNamesShort' => explode( ',', esc_html__( 'Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec', 'wp-inci' ) ),
					'nextText'        => esc_html__( 'Next', 'wp-inci' ),
					'prevText'        => esc_html__( 'Prev', 'wp-inci' ),
					'currentText'     => esc_html__( 'Today', 'wp-inci' ),
					'closeText'       => esc_html__( 'Done', 'wp-inci' ),
					'clearText'       => esc_html__( 'Clear', 'wp-inci' ),
				),
				'time_picker'  => array(
					'timeOnlyTitle' => esc_html__( 'Choose Time', 'wp-inci' ),
					'timeText'      => esc_html__( 'Time', 'wp-inci' ),
					'hourText'      => esc_html__( 'Hour', 'wp-inci' ),
					'minuteText'    => esc_html__( 'Minute', 'wp-inci' ),
					'secondText'    => esc_html__( 'Second', 'wp-inci' ),
					'currentText'   => esc_html__( 'Now', 'wp-inci' ),
					'closeText'     => esc_html__( 'Done', 'wp-inci' ),
					'timeFormat'    => _x( 'hh:mm TT', 'Valid formatting string, as per http://trentrichardson.com/examples/timepicker/', 'wp-inci' ),
					'controlType'   => 'select',
					'stepMinute'    => 5,
				),
			),
			'strings' => array(
				'upload_file'  => esc_html__( 'Use this file', 'wp-inci' ),
				'upload_files' => esc_html__( 'Use these files', 'wp-inci' ),
				'remove_image' => esc_html__( 'Remove Image', 'wp-inci' ),
				'remove_file'  => esc_html__( 'Remove', 'wp-inci' ),
				'file'         => esc_html__( 'File:', 'wp-inci' ),
				'download'     => esc_html__( 'Download', 'wp-inci' ),
				'check_toggle' => esc_html__( 'Select / Deselect All', 'wp-inci' ),
			),
		);
