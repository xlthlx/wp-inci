...
class Sidebar_Manager {
	...
	public function execute_action( $action, $input ) {
		...
		if ( empty( $name ) ) {
			return new \WP_Error( 'name-missing', __( 'Please enter a name for the sidebar.', 'wp-inci' ) );
		}

		$result = new \WP_Error( 'unknown-action', __( 'Unknown action attempted.', 'wp-inci' ) );
		switch ( $action ) {
	...
	public function add_sidebar( $name, $id = '' ) {
		...
		if ( isset( $registered_sidebars[ $id ] ) ) {
			return new \WP_Error( 'sidebar-exists', __( 'Sidebar with the same ID is already registered.', 'wp-inci' ) );
		}
		...
		if ( ! $success ) {
			return new \WP_Error( 'update-failed', __( 'Failed to update option storing your custom sidebars. Please contact support.', 'wp-inci' ) );
		}
	...
	public function remove_sidebar( $id ) {
		...
		if ( isset( $registered_sidebars[ $id ] ) ) {
			...
			return new \WP_Error( 'sidebar-not-found', __( 'Sidebar not found.', 'wp-inci' ) );
		}
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'carbon-sidebar-manager', 'crbSidebarl10n',
			array(
				'add_sidebar' => __( 'Add Sidebar', 'wp-inci' ),
				'enter_name_of_new_sidebar' => __( 'Please enter the name of the new sidebar:', 'wp-inci' ),
				'remove_sidebar_confirmation' => __( 'Are you sure you wish to remove this sidebar?', 'wp-inci' ),
			)
		);
	}
}
